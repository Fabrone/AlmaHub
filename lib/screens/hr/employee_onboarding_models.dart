...
class EmployeeOnboarding {
  ...
  // D. Payroll & Payment Details (retained in model for backward compat)
  final PayrollDetails payrollDetails;

  // E. Academic & Professional Documents (restructured)
  ...
  factory EmployeeOnboarding.fromMap(Map<String, dynamic> map) {
    return EmployeeOnboarding(
      ...
      employmentDetails:
          EmploymentDetails.fromMap(map['employmentDetails'] ?? {}),
      ...
      benefitsInsurance:
          BenefitsInsurance.fromMap(map['benefitsInsurance'] ?? {}),
...
class NextOfKin {
  ...
  Map<String, dynamic> toMap() {
    return {'name': name, 'relationship': relationship, 'contact': contact};
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// D. Payroll & Payment Details  (retained in model for backward compatibility;
//    not shown in the onboarding UI wizard going forward)
// ─────────────────────────────────────────────────────────────────────────────
class PayrollDetails {
  ...
  factory PayrollDetails.fromMap(Map<String, dynamic> map) {
    return PayrollDetails(
      ...
      mpesaDetails: map['mpesaDetails'] != null
          ? MpesaDetails.fromMap(map['mpesaDetails'])
          : null,
    );
  }
}
...
class MpesaDetails {
  ...
  MpesaDetails({this.phoneNumber = '', this.name = ''});

  Map<String, dynamic> toMap() => {'phoneNumber': phoneNumber, 'name': name};

  factory MpesaDetails.fromMap(Map<String, dynamic> map) =>
      MpesaDetails(phoneNumber: map['phoneNumber'] ?? '', name: map['name'] ?? '');
}

// ─────────────────────────────────────────────────────────────────────────────
// E. Academic & Professional Documents  ← RESTRUCTURED
//
//   Section 1 — academicCertificates     : Academic / Professional Certificates
//                                          (merged single list, multi-upload)
//   Section 2 — trainingCertificates     : Training Certificates (NEW)
//                                          (multi-upload)
//   Section 3 — professionalCertificates : Professional Certifications
//                                          (professional body / commission docs,
//                                           multi-upload)
//
//   professionalRegistrations is preserved in fromMap for backward compat with
//   existing Firestore documents but is no longer surfaced in the UI.
// ─────────────────────────────────────────────────────────────────────────────
class AcademicDocuments {
  /// Section 1 – Academic / Professional Certificates (merged)
  final List<DocumentInfo> academicCertificates;

  /// Section 2 – Training Certificates (NEW field)
  final List<DocumentInfo> trainingCertificates;

  /// Section 3 – Professional Certifications (professional body / commission)
  final List<DocumentInfo> professionalCertificates;

  /// Legacy field — kept for backward-compat Firestore reads; not shown in UI
  final Map<String, String> professionalRegistrations;
  ...
  Map<String, dynamic> toMap() {
    return {
      'academicCertificates':
          academicCertificates.map((d) => d.toMap()).toList(),
      'trainingCertificates':
          trainingCertificates.map((d) => d.toMap()).toList(),
      'professionalCertificates':
          professionalCertificates.map((d) => d.toMap()).toList(),
      // Persist the legacy map so old records are not silently wiped
  ...
  factory AcademicDocuments.fromMap(Map<String, dynamic> map) {
    return AcademicDocuments(
      ...
      trainingCertificates: (map['trainingCertificates'] as List<dynamic>?)
          ...
          [],
      professionalCertificates:
          (map['professionalCertificates'] as List<dynamic>?)
                  ?.map((item) => DocumentInfo.fromMap(item))
                  .toList() ??
              [],
      professionalRegistrations:
          Map<String, String>.from(map['professionalRegistrations'] ?? {}),
    );
  }
}
...
class ContractsAndForms {
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'consentDate':
          consentDate != null ? Timestamp.fromDate(consentDate!) : null,
    };
  }
  ...
  factory ContractsAndForms.fromMap(Map<String, dynamic> map) {
    return ContractsAndForms(
      ...
      consentDate: map['consentDate'] != null
          ? (map['consentDate'] as Timestamp).toDate()
          : null,
    );
  }
}
...
class Dependant {
  ...
  Dependant({this.name = '', this.relationship = '', this.dateOfBirth});
  ...
  factory Dependant.fromMap(Map<String, dynamic> map) => Dependant(
        name: map['name'] ?? '',
        relationship: map['relationship'] ?? '',
        dateOfBirth: map['dateOfBirth'] != null
            ? (map['dateOfBirth'] as Timestamp).toDate()
...
class Beneficiary {
  ...
  factory Beneficiary.fromMap(Map<String, dynamic> map) => Beneficiary(
        name: map['name'] ?? '',
        relationship: map['relationship'] ?? '',
        contact: map['contact'] ?? '',
...
class IssuedEquipment {
  ...
  factory IssuedEquipment.fromMap(Map<String, dynamic> map) => IssuedEquipment(
